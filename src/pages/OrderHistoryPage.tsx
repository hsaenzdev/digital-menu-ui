...
export const OrderHistoryPage: React.FC = () => {
  ...
  return (
    <div className="h-screen flex flex-col bg-gradient-to-br from-fire-500 via-fire-600 to-ember-600 overflow-hidden">
        ...
        <div className="p-3 pb-20 space-y-3">
          ...
          {!loading && orders.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-gray-600 mb-6 font-medium text-sm">Your order history will appear here once you place your first order!</p>
              <button 
                className="bg-gradient-to-r from-fire-600 to-ember-600 text-white font-semibold text-sm py-2.5 px-6 rounded-lg hover:from-fire-700 hover:to-ember-700 transition-all shadow-lg"
                onClick={() => navigate(`/${customerId}`)}
              >
          ...
          {!loading && orders.length > 0 && (
            <div className="space-y-2.5">
              ...
              {orders.map((order) => (
                <div key={order.id} className="bg-white rounded-xl shadow-md p-3 border border-fire-200 hover:shadow-lg transition-shadow">
      ...
      {/* Fixed Action Button - Removed (using floating home button instead) */}
    </div>
  )
}